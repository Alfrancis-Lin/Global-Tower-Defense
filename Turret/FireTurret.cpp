...
FireTurret::FireTurret(float x, float y) : Turret("play/tower-base.png", "play/turret-3.png", x, y, 300, Price, 0.5) {
...
void FireTurret::Update(float deltaTime) {
            ...
            */
            if (diff.Magnitude() <= CollisionRadius) {
                ...
                if (enemy) {
                    ...
                    if (Targets.size() == 5) break; // 最多 5 個
                }
            }
        }
    ...
    if (!Targets.empty()) {
        ...
        float maxRotateRadian = rotateRadian * deltaTime * 3;
